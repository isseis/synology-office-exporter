...
from drive_export import OfficeFileFetcher, SynologyDriveEx, save_bytesio_to_file
...
class TestOfficeFileFetcher(unittest.TestCase):
    ...
    def test_execute(self, mock_save_bytesio_to_file):
        ...
        mock_synd.list_folder.return_value = {
            ...
            'data': {
                'items': [
                    {'content_type': 'document', 'encrypted': False, 'name': 'test.osheet', 'display_path': 'path/to/test.osheet', 'file_id': '123'},
                    {'content_type': 'dir', 'encrypted': False, 'name': 'folder', 'display_path': 'path/to/folder', 'file_id': '456'}
                ]
            }
        }
